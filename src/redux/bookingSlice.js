...
export const fetchUserBookings = createAsyncThunk('bookings/fetchUser', async (_, { getState, rejectWithValue }) => {
    try {
        const { token } = getState().auth;
        const config = { headers: { Authorization: `Bearer ${token}` } };
...
export const fetchAllBookings = createAsyncThunk('bookings/fetchAll', async (_, { getState, rejectWithValue }) => {
    try {
        const { token } = getState().auth;
        const config = { headers: { Authorization: `Bearer ${token}` } };
...
export const deleteBooking = createAsyncThunk('bookings/delete', async (bookingId, { getState, rejectWithValue }) => {
    try {
        const { token } = getState().auth;
        const config = { headers: { Authorization: `Bearer ${token}` } };
...
const bookingSlice = createSlice({
    ...
    reducers: {
        updateBookingFromSocket: (state, action) => {
            ...
            if (adminIndex !== -1) {
                state.allBookings[adminIndex] = updatedBooking;
            }
        },
        // --- REDUCERS AJOUTÉS POUR LE TEMPS RÉEL ---
...
export const { updateBookingFromSocket, addBooking, removeBooking } = bookingSlice.actions;
